...
/**
  ...
  */
object CityPassengerFlow extends SparkTool {
  /**
    * 在run方法中编写spark的业务逻辑
    *
    * @param args 运行参数
  ...
  override def run(args: Array[String]): Unit = {
    ...
    //天分区
    val dayId = args(0)
    val cityInputPath = Constants.CITY_INPUT_PATH + Constants.PARTITION_NAME_DAY + dayId
    Logger.info(s"市游客表输入路径:$cityInputPath")
    ...
    /**
      ...
      */
    val cityDF = sqlContext.read.parquet(cityInputPath)
    ...
    /**
      * 省游客表与用户画像表关联
      */
    val joinDF = cityDF.join(userTagDF, "mdn")
    ...
    def saveDataByColumn(column: String) = {
      if (column != null) {
        IndexToRedis.saveDataToRedis(joinDF, dayId, "d_city_id", column)
      } else {
        IndexToRedis.saveDataToRedis(joinDF, dayId, "d_city_id")
      }
    }
